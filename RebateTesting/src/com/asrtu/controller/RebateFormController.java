...
public class RebateFormController {
		...
		if ("refreshPage".equals(rebateBean.getPageAction())) {
			...
			for (RebateInvoiceBean rebateInvoiceBean : rebateInvoiceBeanList) {
				...
				if (rebateInvoiceBean.getInvoicePayment() > 0) {
					...
					// Deduct Principal outstanding than Tax outstanding
					Invoice invoice = new Invoice();
					invoice.setInvoiceNumber(rebateInvoiceBean.getInvoiceNumber());
					
					// Get Invoice Data
					
					Invoice invoiceToUpdate = invoiceService.getInvoice(rebateInvoiceBean.getInvoiceNumber());
					
					if(rebateInvoiceBean.getInvoicePayment() < rebateInvoiceBean.getRebateRemainingOutstanding()){
						
						if(rebateInvoiceBean.getInvoicePayment() < rebateInvoiceBean.getRebatePrRemainingOutstanding()){
							invoice.setRebatePrRemainingOutstanding(rebateInvoiceBean.getRebatePrRemainingOutstanding() - rebateInvoiceBean.getInvoicePayment());
							invoice.setRebateTaxRemainingOutstanding(rebateInvoiceBean.getRebateTaxRemainingOutstanding());
						} else{
							invoice.setRebatePrRemainingOutstanding(0f);
							invoice.setRebateTaxRemainingOutstanding(rebateInvoiceBean.getRebateTaxRemainingOutstanding() - rebateInvoiceBean.getInvoicePayment() + rebateInvoiceBean.getRebatePrRemainingOutstanding());
						}
					}else {
						
						invoice.setRebatePrRemainingOutstanding(0f);
						invoice.setRebateTaxRemainingOutstanding(0f);
					}
	...
	private RebateForm prepareModelForInvoicePayment(RebateFormBean rebateBean, RebateInvoiceBean rebateInvoiceBean) throws Exception {
		RebateForm rebate = new RebateForm();
		rebate.setTransactionNo(rebateBean.getTransactionNo());
		rebate.setTransactionDate(rebateBean.getTransactionDate());
		rebate.setReceiptDate(rebateBean.getReceiptDate());
		...
		// interest on rebate for Invoice Payment
				...
				Float intOnRebateEligibleAmount = 0f;
				Float totalTaxPercentage = rebateInvoiceBean.getTotalTaxPercentage();
				...
				if(null != rebateInvoiceBean.getInvoicePayment() && rebateInvoiceBean.getInvoicePayment() >0){
					...
					intOnRebateEligibleAmount = 365*100*rebateInvoiceBean.getInvoicePayment()/(36500+totalTaxPercentage*totalDaysForSt);
					
					
					if(rebateInvoiceBean.getRebateType().equalsIgnoreCase("Regular Rebate")){
						
						interesOnRebateInvoice = intOnRebateEligibleAmount*totalDaysForSt*12/36500;
						
					} else if (rebateInvoiceBean.getRebateType().equalsIgnoreCase("Rebate Reconciliation")){
						
						interesOnRebateInvoice = intOnRebateEligibleAmount*totalDaysForSt*24/36500;
					} else {
	...
	private RebateForm prepareModelForNonInvoicePayment(RebateFormBean rebateBean) throws Exception {
		RebateForm rebate = new RebateForm();
		rebate.setTransactionNo(rebateBean.getTransactionNo());
		rebate.setTransactionDate(rebateBean.getTransactionDate());
		rebate.setReceiptDate(rebateBean.getReceiptDate());
	...
	private RebateFormBean prepareRebateInvoicesList(List<Invoice> invoices, RebateFormBean rebateBean) {
		...
		for (Invoice invoice : invoices) {
			...
			if(null !=invoice && null != invoice.getRebateRemainingOutstanding() && invoice.getAsrtuInvoiceNumber()>0 ){
				
				if(invoice.getRebateRemainingOutstanding() > 0){
					rebateInvoiceBean = new RebateInvoiceBean(invoice.getInvoiceNumber(),invoice.getAsrtuInvoiceNumber(),  invoice.getRebateOutstanding(), invoice.getRebateRemainingOutstanding());
					rebateInvoiceBean.setQuarter(invoice.getQuarter());
					rebateInvoiceBean.setRebateType(invoice.getRebateType());
					rebateInvoiceBean.setTotalTaxPercentage(invoice.getTotalTaxPercentage());
					
					rebateInvoiceBean.setRebatePrRemainingOutstanding(invoice.getRebatePrRemainingOutstanding());
					rebateInvoiceBean.setRebateTaxRemainingOutstanding(invoice.getRebateTaxRemainingOutstanding());
